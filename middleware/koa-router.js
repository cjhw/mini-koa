...
class Router {
  ...
  compose(matchLayers, ctx, next) {
    function dispatch(index) {
      if (index === matchLayers.length) {
        return next() // 如果走到最后就从路由中间件出去，或者一个没有匹配到也出去
      }
      return Promise.resolve(
        matchLayers[index].callback(ctx, () => dispatch(index + 1))
      )
    }
    return dispatch(0)
  }
  routes() {
    ...
    return async (ctx, next) => {
      ...
      let matchLayers = this.stack.filter((layer) =>
